...
public class PopupTargetHistory : MonoBehaviour
{
    ...
    {
        target_object.SetInfo(CategoryManager.Instance.CategoryDic[target.category].icon, target.category, target.goal, target.goal - target.current_amount, (target.goal - target.current_amount) / target.goal, CategoryManager.Instance.CategoryDic[target.category].color, null);
        DeleteContainer();
